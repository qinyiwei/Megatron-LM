...
class MoELayerMonitor:
    ...
    def collect_forward_metrics(
        ...
        if self.should_log_level(step):
            ...
            drop_rate = float(dropped_tokens) / (total_assigned_tokens + 1e-10) if total_assigned_tokens > 0 else 0.0
            self.level_0_metrics = {
                ...
                'drop_rate': drop_rate,
                'total_assigned_tokens': total_assigned_tokens,
            }
    ...
    def write_to_tensorboard(self, writer, step: int):
        ...
        if self.level_0_metrics:
            ...
            writer.add_scalar(f'{prefix}_expert/total_assigned_tokens', 
                                self.level_0_metrics['total_assigned_tokens'], step)
...
def ingest_moe_monitoring_forward_metrics(step: int):
    ...
    for layer_id, entry in buffer.items():
        ...
        total_assigned_tokens = expert_counts.sum().item()
        ...
        layer_monitor.collect_forward_metrics(
            ...
            dropped_tokens=int(max(0.0, total_assigned_tokens - kept_tokens)),
            total_assigned_tokens=int(total_assigned_tokens),
            step=step,
        )
